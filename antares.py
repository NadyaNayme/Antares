...
where_clauses = []
for index in parsed_sql:
    where_indices = [n for n, clause in enumerate(index) if clause =='WHERE' or clause == 'AND' ]
    where_clauses.append(where_indices)

orderby_clauses = []
for index in parsed_sql:
    orderby_indices = [n for n, clause in enumerate(index) if clause =='ORDER' ]
...
check_join_clauses = [i for i, val in enumerate(join_clauses) if val != [] and val == [0] or val == [1]]
for indice in check_join_clauses:
    if parsed_sql[indice].__len__() == 10:
        new_join = {'type': ' '.join(map(str,parsed_sql[indice][0:2])), 'db': parsed_sql[indice][3], 'alias': parsed_sql[indice][5], 'id1': parsed_sql[indice][7], 'id2': parsed_sql[indice][9] }
        join.append(new_join)
    elif parsed_sql[indice].__len__() == 9:
        new_join = {'type': parsed_sql[indice][0], 'db': parsed_sql[indice][2], 'alias': parsed_sql[indice][4], 'id1': parsed_sql[indice][6], 'id2': parsed_sql[indice][8] }
        join.append(new_join)
    else:
        new_join = {'type': parsed_sql[indice][0], 'db': parsed_sql[indice][2], 'alias': None, 'id1': parsed_sql[indice][4], 'id2': parsed_sql[indice][6] }
        join.append(new_join)
...
check_apply_clauses = [i for i, val in enumerate(apply_clauses) if val != [] and val == [0] or val == [1]]
for indice in check_apply_clauses:
    if parsed_sql[indice].__len__() > 7:
        new_apply = {'type': parsed_sql[indice][0], 'db': parsed_sql[indice][-1:-2], 'alias': parsed_sql[indice][-1], 'sql': ' '.join(map(str,parsed_sql[indice][2:-2])) }
        apply.append(new_apply)
    else:
        new_apply = {'type': parsed_sql[indice][0], 'db': parsed_sql[indice][2], 'alias': None, 'sql': ' '.join(map(str,parsed_sql[indice][4:-1])) }
        apply.append(new_apply)

# Parse simple WHERE and AND clauses. Name is second index, match_type is third, and the value to match is fourth.
check_where_clauses = [i for i, val in enumerate(where_clauses) if val != [] and val == [0]]
for indice in check_where_clauses:
    if parsed_sql[indice][2] == "=":
        new_where = { 'name': parsed_sql[indice][1], 'match_type': match_dict(parsed_sql[indice][2]), 'value': parsed_sql[indice][3] }
        where.append(new_where)
    elif parsed_sql[indice][2] == "IS" and parsed_sql[indice][3] == "NOT" and parsed_sql[indice][4] == "NULL":
        new_where = { 'name': parsed_sql[indice][1], 'match_type': ' '.join(map(str,parsed_sql[indice][2:4])), 'value': parsed_sql[indice][4] }
        where.append(new_where)
    elif parsed_sql[indice][2] == "IS" and parsed_sql[indice][3] == "NULL":
        new_where = { 'name': parsed_sql[indice][1], 'match_type': ' '.join(map(str,parsed_sql[indice][2:3])), 'value': parsed_sql[indice][3] }
        where.append(new_where)
    else:
        print("Failure to parse WHERE clause correctly")

# Parse ORDER BY clause. Grab all index after the second
check_orderby_clauses = [i for i, val in enumerate(orderby_clauses) if val != []]
...
if select_columns:
    select_line = 'select: \'{fields}\','.format(fields = ' '.join(map(str, select_columns)))
if top_value:
    top_line = 'top: {tvalue}'.format(tvalue = top_value)
if db['from'] and db['alias']:
    from_line = 'from: \'{database}\',\n\talias: \'{alias}\','.format(database = db['from'], alias = db['alias'])
elif db['from']:
...
if join.__len__() > 0:
    for item in join:
        ...
        join_id1 = item['id1']
        join_id2 = item['id2']
        join_on = '[{{ id1: \'{jid1}\', id2: \'{jid2}\' }}]'.format(jid1 = join_id1, jid2 = join_id2)
        join_lines.append('  {{\n\t\ttype: \'{jtype}\',\n\t\tto: \'{jfrom}\',\n\t\talias: \'{jalias}\',\n\t\ton: {jon}\n\t  }}'.format(jtype = join_type, jfrom = join_from, jalias = join_alias, jon = join_on))
...
if apply.__len__() > 0:
    for item in apply:
        ...
        apply_lines.append('  {{\n\t\ttype: \'{atype}\',\n\t\tto: \'\n\t\t  {ato}\n\t\t\',\n\t\talias: \'{aalias}\',\n\t  }}'.format(atype = apply_type, ato = apply_to, aalias = apply_alias))
...
if where.__len__() > 0:
    for item in where:
        ...
        where_lines.append('  {{ name: \'{wname}\', match: \'{wtype}\', value: \'{wvalue}\' }}'.format(wname = where_name, wtype = where_type, wvalue = where_value.replace('\'','')))
        
coral_sql = 'datasource: {{\n\t{sline}\n\t{tline}\n\t{fline}\n\tjoin: [\n\t{jlines}\n\t],\n\tapply: [\n\t{alines}\n\t],\n\twhere: [\n\t{wlines}\n\t],\n\torderby: \'{oby}\'\n}}'.format(sline = select_line, tline = top_line, fline = from_line, jlines = ',\n\t'.join(map(str, join_lines)), alines = ',\n\t'.join(map(str, apply_lines)), wlines = ',\n\t'.join(map(str, where_lines)), oby = ' '.join(map(str,orderby)))
